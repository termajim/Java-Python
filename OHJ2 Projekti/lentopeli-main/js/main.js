...
// Function to fetch a list of airports
async function fetchAirports() {
    try {
        ...
        return data.airports; // Assuming the response is in the format { airports: [...] }
    } catch (error) {
...
//Randomly selects a airport
async function selectRandomAirport() {
  const airports = await fetchAirports();
  if (airports.length === 0) {
    console.error('No airports available.');
    return;
  }
...
async function startGame() {
    const randomAirport = await selectRandomAirport();
    if (randomAirport) {
        const startLoc = randomAirport; // Assuming the random airport is returned as a string
        // Proceed with your game initialization using the randomly selected airport
    }
}

// Call startGame to begin the game with a random airport
startGame();
